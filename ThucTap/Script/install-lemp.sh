...
#  Includes  : Nginx (HTTP) · MySQL 8 · PHP 8.1-FPM
...
BANNER
echo -e "${NC}"
echo -e "  ${CYAN}Server IP:${NC} ${SERVER_IP}  |  ${CYAN}PHP:${NC} ${PHP_VER}  |  ${CYAN}HTTP:${NC} Port 80"
...
#  NGINX SITE CONFIG
# ─────────────────────────────────────────────────────────────
info "Writing site configuration (HTTP port 80)..."
cat > "$SITE_CONF" << SITECONF
# ============================================================
#  LEMP Site — ${SERVER_IP}
#  HTTP · FastCGI Cache · Detailed Logging
# ============================================================
...
success "Site config enabled on HTTP port 80"
  ...
  <div class="card card-blue">
    ...
    <div class="row-list">
      <div class="row-item">
        ...
        <span class="row-val"><?= $protocol ?></span>
      </div>
      <div class="row-item">
...
<div class="card">
  ...
  <div style="font-family:var(--mono);font-size:11px;line-height:2;color:#8892a4">
    ...
    <div style="margin-top:8px;color:var(--muted)">All endpoints restricted to <?= $serverIP ?> via HTTP</div>
  </div>
</div>
</div>
...
[[ "$HTTP_CODE" == "200" ]] && success "HTTP status page: HTTP $HTTP_CODE" \
                             || warn    "HTTP check returned: HTTP $HTTP_CODE (check nginx status)"
...
echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  ${CYAN}Status Page${NC}   →  http://${SERVER_IP}/"
echo -e "  ${CYAN}Nginx Status${NC}  →  http://${SERVER_IP}/nginx-status"
echo -e "  ${CYAN}FPM Status${NC}   →  http://${SERVER_IP}/fpm-status"
echo -e "  ${CYAN}PHP Info${NC}      →  http://${SERVER_IP}/phpinfo.php"
echo -e "  ${CYAN}Cache Purge${NC}  →  http://${SERVER_IP}/cache-purge"
echo ""
